...
export default function PreTestNav() {
  ...
  return (
    <nav
      ...
      className="mt-7 px-5 md:mx-auto md:w-full md:max-w-330 md:py-0"
    >
      <ol className="flex gap-10">
        <li
          onClick={() => router.push("/counselGuide")}
          className="flex cursor-pointer items-center gap-1"
        >
          <NumberBadge number={1} state={isCounselGuide ? "selected" : "unselected"} />
          <span className="text-label-m md:text-title-l text-neutral-500">
            우리 회사 상담 가이드 등록
          </span>
        </li>
        <li
          onClick={() => router.push("/callAnalytics")}
          className="flex cursor-pointer items-center gap-1"
        >
          <NumberBadge number={2} state={isCallAnalytics ? "selected" : "unselected"} />
          <span className="text-label-m md:text-title-l text-neutral-500">통화 녹음 업로드</span>
        </li>
      </ol>
    </nav>
  );
